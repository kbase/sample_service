...
from common import check_kafka_messages
...
def test_kafka_notifier_new_sample(kafka_host):
    ...
    try:
        ...
        check_kafka_messages(
            kafka_host,
            [{'event_type': 'NEW_SAMPLE', 'sample_id': str(sample_id), 'sample_ver': 6}],
            topic)
    finally:
        kn.close()
...
def test_kafka_notifier_notify_new_sample_version_fail(kafka_host):
    ...
    def kafka_notifier_notify_new_sample_version_fail(notifier, sample, version, expected):
        with raises(Exception) as got:
            notifier.notify_new_sample_version(sample, version)
        assert_exception_correct(got.value, expected)

    kafka_notifier_notify_new_sample_version_fail(kn, None, 1, ValueError(
        'sample_id cannot be a value that evaluates to false'))
    kafka_notifier_notify_new_sample_version_fail(kn, uuid.uuid4(), 0, ValueError(
        'sample_ver must be > 0'))
    kafka_notifier_notify_new_sample_version_fail(kn, uuid.uuid4(), -3, ValueError(
        'sample_ver must be > 0'))

    kn.close()
    kafka_notifier_notify_new_sample_version_fail(kn, uuid.uuid4(), 1, ValueError(
        'client is closed'))
...
def test_kafka_notifier_notify_acl_change_fail(kafka_host):
    ...
    kafka_notifier_notify_acl_change_fail(kn, None, ValueError(
        'sample_id cannot be a value that evaluates to false'))

    kn.close()
    kafka_notifier_notify_acl_change_fail(kn, uuid.uuid4(), ValueError(
        'client is closed'))
...
def test_kafka_notifier_new_link_fail(kafka_host):
    ...
    kafka_notifier_new_link_fail(kn, None, ValueError(
        'link_id cannot be a value that evaluates to false'))

    kn.close()
    kafka_notifier_new_link_fail(kn, uuid.uuid4(), ValueError(
        'client is closed'))
...
def test_kafka_notifier_expired_link_fail(kafka_host):
    ...
    kafka_notifier_expired_link_fail(kn, None, ValueError(
        'link_id cannot be a value that evaluates to false'))

    kn.close()
    kafka_notifier_expired_link_fail(kn, uuid.uuid4(), ValueError(
        'client is closed'))
