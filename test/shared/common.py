...
from test_constants import TEST_COL_SCHEMA, TEST_COL_WS_OBJ_VER, TEST_COL_DATA_LINK, TEST_COL_NODE_EDGE, TEST_COL_NODES, \
    TEST_COL_VER_EDGE, TEST_COL_VERSION, TEST_COL_SAMPLE, KAFKA_TOPIC, KAFKA_PORT, TOKEN_WS_READ_ADMIN, \
    MOCK_SERVICES_PORT, TEST_PWD, TEST_USER, TEST_DB_NAME, ARANGODB_PORT, TOKEN_SERVICE, TOKEN2
...
def replace_acls(url, sample_id, token, acls, as_admin=0, debug=False):
    response = requests.post(url, headers=get_authorized_headers(token),
                             json=make_rpc('replace_sample_acls', [{
                                 'id': sample_id,
                                 'acls': acls,
                                 'as_admin': as_admin
                             }]))
...
def create_generic_sample(url, token):
    ret = requests.post(url, headers=get_authorized_headers(token), json=make_rpc('create_sample', [CASE_01]))
    assert ret.ok is True
...
def get_sample(url, token, sample_id, version=1, as_admin=False):
    return rpc_call(url, token, 'get_sample', [{
        'id': str(sample_id),
        'version': version,
        'as_admin': as_admin}])
...
def get_sample_result(url, token, sample_id, version=1, as_admin=False):
    return rpc_call_result(url, token, 'get_sample', [{
        'id': str(sample_id),
        'version': version,
        'as_admin': as_admin}])
...
def create_deploy_cfg():
    ...
    cfg[ss]['auth-service-url'] = (f'http://mockservices:{MOCK_SERVICES_PORT}/services/auth/' +
    ...
    temp_file, temp_file_path = tempfile.mkstemp('.cfg', 'metaval-', dir=test_utils.get_temp_dir(), text=True)
    os.close(temp_file)
    ...
    deploy_file, deploy_file_path = tempfile.mkstemp('.cfg', 'deploy-', dir=test_utils.get_temp_dir(), text=True)
...
def create_sample_assert_result(url, token, params, expectations={'version': 1}, debug=False):
...
def assert_fail_create_sample(url, token, params, error_message):
    response = requests.post(url, headers=get_authorized_headers(token), json=make_rpc('create_sample', [params]))
