...
from common import (
    replace_acls, assert_get_sample, assert_create_sample, clear_kafka_messages, check_kafka_messages,
    get_sample_assert_result, create_sample_assert_result, sample_params_to_sample, create_sample_assert_error,
    get_samples_assert_result
)
from test_cases import CASE_01, CASE_02, CASE_03, CASE_00
#
# Simple sample creation cases. Does not assume anything else -- see later tests for more complex scenarios.
#
from test_constants import USER1, TOKEN1, TOKEN3, USER3, USER4, TOKEN2, USER2, TOKEN4
...
def test_create_sample_prefix_field(sample_service):
    ...
    #
    result = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_03,
                                         {'version': 1})
    get_sample_assert_result(sample_service['url'], TOKEN1, {'id': result['id'], 'version': 1},
                             sample_params_to_sample(CASE_03,
                                                     {'user': USER1,
                                                      'id': result['id'],
                                                      'version': 1}))
...
def test_create_sample_string_field_multiple_versions(sample_service):
    ...
    for expected_sample_version in range(1, 5):
        result = create_sample_assert_result(sample_service['url'], TOKEN1, params,
                                             {'version': expected_sample_version})
        get_sample_assert_result(sample_service['url'], TOKEN1,
                                 {'id': result['id'], 'version': expected_sample_version},
                                 sample_params_to_sample(params, {'user': USER1,
                                                                  'id': result['id'],
                                                                  'version': expected_sample_version}))
...
def test_create_sample_then_get_sample(sample_service):
    result = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_01, {'version': 1})
    get_sample_assert_result(sample_service['url'], TOKEN1, {'id': result['id'], 'version': 1},
                             sample_params_to_sample(CASE_01, {'user': USER1,
                                                               'id': result['id'],
                                                               'version': 1}))
...
def test_create_sample_string_field_two_versions_kafka(sample_service, kafka_host):
    ...
    # First sample
    result = create_sample_assert_result(sample_service['url'], TOKEN1, params, {'version': 1})
    get_sample_assert_result(sample_service['url'], TOKEN1, {'id': result['id'], 'version': 1},
                             sample_params_to_sample(CASE_01, {'user': USER1,
                                                               'id': result['id'],
                                                               'version': 1}))
    ...
    for sample_version in range(2, 4):
        result = create_sample_assert_result(sample_service['url'], TOKEN1, params, {'version': sample_version})
        get_sample_assert_result(sample_service['url'], TOKEN1, {'id': result['id'], 'version': sample_version},
                                 sample_params_to_sample(CASE_01, {'user': USER1,
                                                                   'id': result['id'],
...
def test_create_sample_as_admin_impersonate_user(sample_service):
    create_sample_as_admin(sample_service['url'], TOKEN2, '     ' + USER4 + '      ', TOKEN4, USER4)
...
def test_create_sample_fail_no_nodes(sample_service):
    ...
    create_sample_assert_error(sample_service['url'], TOKEN1, params, {'message': error_message})
...
def test_create_sample_fail_bad_metadata(sample_service):
    def create_sample_fail_bad_metadata(controlled_metadata, expected_error_message, source_meta=None):
        ...
        create_sample_assert_error(sample_service['url'], TOKEN1, params, {'message': expected_error_message})
    ...
    cases = [
        ...
        # Value set to None is a different error (although the message above implies otherwise)
        {
            ...
            'expected_error_message': (
                "Sample service error code 30001 Illegal input parameter: Node at index 0's controlled metadata entry "
                "does not have a dict as a value at key foo"
            )
        },
        # The value of a value key cannot be null (aka None here)
        {
            ...
            'expected_error_message': (
                "Sample service error code 30001 Illegal input parameter: Node at index 0's controlled metadata "
                "entry does not have a primitive type as the value at foo/value"
            )
        },
        ...
        {
            'controlled_metadata': {'stringlentest': {'foooo': 'barrr', 'spcky': 'baz'}},
            'expected_error_message': (
        ...
        # Tests the max-len constraint for a prefix validator
        {
            ...
            'expected_error_message': (
                "Sample service error code 30010 Metadata validation failed: Node at index 0: "
                "Prefix validator bar, key bark: {'subkey': 'fail_plz', 'message': 'Metadata value at key fail_plz is longer than max length of 10'}"
            )
        },
    ...
    for case in cases:
        create_sample_fail_bad_metadata(case['controlled_metadata'], case['expected_error_message'],
...
def test_create_sample_fail_permissions(sample_service):
    ...
    create_sample_assert_error(sample_service['url'], TOKEN2, params,
...
def create_sample_fail_admin_as_user(url, user, expected_error_message):
    ...
    create_sample_assert_error(url, TOKEN2, params,
                               {'message': expected_error_message})


def test_create_sample_fail_admin_bad_user_name(sample_service):
    create_sample_fail_admin_as_user(sample_service['url'], 'bad\tuser', (
        'Sample service error code 30001 Illegal input parameter: '
...
def test_create_sample_fail_admin_no_such_user(sample_service):
    create_sample_fail_admin_as_user(sample_service['url'], 'impostor', (
...
def test_create_sample_fail_admin_permissions(sample_service):
    ...
    expected_error_message = (
        'Sample service error code 20000 Unauthorized: '
        f'User {USER3} does not have the necessary administration privileges to run method create_sample'
    )
    create_sample_assert_error(sample_service['url'], TOKEN3, params,
                               {'message': expected_error_message})
...
def test_create_and_get_sample_with_version(sample_service, kafka_host):
    ...
    # version 1
    sample = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_01, {'version': 1})
    sample_id = sample['id']
    ...
    sample2 = create_sample_assert_result(sample_service['url'], TOKEN1, params2, {'version': 2})

    # get version 1
    get_sample_assert_result(sample_service['url'], TOKEN1, {'id': sample_id, 'version': 1},
                             sample_params_to_sample(CASE_01, {'user': USER1,
                                                               'id': sample_id,
                                                               'version': 1}))

    # get version 2
    get_sample_assert_result(sample_service['url'], TOKEN1, {'id': sample_id, 'version': 2},
                             sample_params_to_sample(CASE_01, {'user': USER1,
                                                               'id': sample_id,
                                                               'version': 2}))
...
def test_create_and_get_samples(sample_service, kafka_host):
    ...
    # first sample
    sample1 = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_01, {'version': 1})

    # second sample
    sample2 = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_02, {'version': 1})

    # get both samples
    get_samples_assert_result(sample_service['url'], TOKEN1, {'samples': [{'id': sample1['id'], 'version': 1},
                                                                          {'id': sample2['id'], 'version': 1}]},
                              [sample_params_to_sample(CASE_01, {'user': USER1,
                                                                 'id': sample1['id'],
                                                                 'version': 1}),
                               sample_params_to_sample(CASE_02, {'user': USER1,
                                                                 'id': sample2['id'],
                                                                 'version': 1})])
