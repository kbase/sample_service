...
from SampleService.core.acls import SampleACL, SampleACLDelta, SampleACLOwnerless
...
from SampleService.core.workspace import UPA, WS, DataUnitID, WorkspaceAccessType
from testing.shared.common import dt, now_fun, nw
...
def test_init_fail_bad_args():
    ...
    mock_sample_storage = create_autospec(
        ArangoSampleStorage, spec_set=True, instance=True
    )
    mock_user_lookup = create_autospec(KBaseUserLookup, spec_set=True, instance=True)
    mock_metadata_validator_set = create_autospec(
        MetadataValidatorSet, spec_set=True, instance=True
    )
...
def test_get_samples():
    def _get_samples(user, version, as_admin, public_read=False):
        ...
        samples = Samples(
            ...
            meta,
            ws,
            uuid_gen=lambda: UUID("1234567890abcdef1234567890fbcdef"),
        )
...
def test_replace_sample_acls_with_owner_change_fail_5_times():
    ...
    assert storage.replace_sample_acls.call_args_list == [
        (
            (
                UUID("1234567890abcdef1234567890abcde0"),
                SampleACL(u(f"someuser{x}"), nw(), read=[u("c")]),
            ),
            {},
        )
        for x in range(5)
...
def _get_links_from_sample(user, public_read=False):
    ...
    storage.get_links_from_sample.assert_called_once_with(
        SampleAddress(UUID("1234567890abcdef1234567890abcdee"), 3), [7, 90, 106], now()
    )
...
def test_get_links_from_sample_as_admin():
    ...
    storage.get_links_from_sample.assert_called_once_with(
        SampleAddress(UUID("1234567890abcdef1234567890abcdee"), 3), None, now()
    )
