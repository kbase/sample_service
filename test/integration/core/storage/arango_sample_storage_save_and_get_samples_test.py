...
from common import TEST_COL_SAMPLE, TEST_DB_NAME, TEST_COL_NODES, TEST_COL_SCHEMA, \
    TEST_COL_DATA_LINK, TEST_COL_WS_OBJ_VER, TEST_COL_NODE_EDGE, TEST_COL_VER_EDGE, TEST_COL_VERSION, dt, make_uuid
...
def test_save_sample_fail_duplicate(samplestorage):
    ...
    assert samplestorage.save_sample(
        SavedSample(sample_id, UserID('user1'), [TEST_NODE], dt(1), 'foo')) is True

    assert samplestorage.save_sample(
        SavedSample(sample_id, UserID('user1'), [TEST_NODE], dt(1), 'bar')) is False
...
def test_save_sample_fail_duplicate_race_condition(samplestorage):
    ...
    sample_id = uuid.uuid4()
    assert samplestorage.save_sample(
        SavedSample(sample_id, UserID('user'), [TEST_NODE], dt(1), 'foo')) is True
    ...
    assert samplestorage._save_sample_pt2(
        SavedSample(sample_id, UserID('user'), [TEST_NODE], dt(1), 'bar')) is False
...
def test_get_sample_fail_no_such_version(samplestorage):
    ...
    assert samplestorage.save_sample_version(
        SavedSample(sample_id, UserID('user2'), [TEST_NODE], dt(1), 'bar')) == 2
...
def test_get_sample_fail_no_version_doc_2_versions(samplestorage):
    ...
    assert samplestorage.save_sample(
        SavedSample(sample_id, UserID('user'), [TEST_NODE], dt(1), 'foo')) is True
    assert samplestorage.save_sample_version(
        SavedSample(sample_id, UserID('user'), [TEST_NODE], dt(1), 'bar')) == 2
