...
from common import clear_kafka_messages, assert_create_sample, create_link, rpc_call_result, check_kafka_messages, \
    replace_acls, create_sample, assert_error_create_link, assert_error_rpc_call, assert_result_rpc_call, \
    get_current_epochmillis, assert_result_create_link, make_expected_sample, get_sample_node_id
from service_client import ServiceClient
from test_cases import CASE_01, CASE_02, CASE_04
from test_constants import USER4, TOKEN4, USER3, TOKEN3, TOKEN5, USER1, TOKEN1, TOKEN2, USER2
from test_utils import assert_ms_epoch_close_to_now
...
def _expire_data_link(url, dataid, kafka_host):
    ...
    link_id1 = create_link(url, TOKEN3,
                           {'id': sample_id,
                            'version': 1,
                            'node': 'root',
                            'upa': upa,
    ...
    link_id2 = create_link(url, TOKEN3,
                           {'id': sample_id,
                            'version': 1,
                            'node': 'root',
                            'upa': upa,
    ...
    assert_result_rpc_call(url, TOKEN4,
                           'expire_data_link',
                           [{'upa': upa,
                             'dataid': dataid,
                             'as_user': USER1}],
    ...
    # check links
    result = rpc_call_result(url, TOKEN4,
...
def _expire_data_link_as_admin(url, sample_expire_user, expected_user, kafka_host):
    ...
    # create links
    link_id = assert_result_create_link(url, sample_creator_token,
                                        {'id': sample_id,
                                         'version': 1,
                                         'node': 'root',
                                         'upa': upa,
    ...
    # expire link
    assert_result_rpc_call(url, sample_admin_token,
                           'expire_data_link',
                           [{'upa': upa,
                             'dataid': 'duidy',
                             'as_admin': 1,
                             'as_user': sample_expire_user}],
                           None)

    # check links
    result = rpc_call_result(url, sample_reader_user,
                             'get_data_links_from_data',
                             [{'upa': upa,
                               'effective_time': get_current_epochmillis() - 500}])
...
def test_create_links_and_get_links_from_sample_basic(sample_service, kafka_host):
    ...
    link_id1 = create_link(sample_service['url'], TOKEN3,
                           {'id': sample_id1,
                            'version': 1,
                            'node': 'root',
                            'upa': '3/2/2',
    ...
    link_id2 = create_link(sample_service['url'], TOKEN3,
                           {'id': sample_id1,
                            'version': 1,
                            'node': 'root',
                            'upa': '3/1/1',
    ...
    link_id3 = create_link(sample_service['url'], TOKEN4,
                           {'id': sample_id2,
                            'version': 1,
                            'node': 'root2',
                            'upa': '3/2/1',
    ...
    result = rpc_call_result(sample_service['url'], TOKEN3,
    ...
    result = rpc_call_result(sample_service['url'], TOKEN4,
                             'get_data_links_from_sample',
    ...
    check_kafka_messages(kafka_host,
                         [
...
def test_update_and_get_links_from_sample(sample_service, kafka_host):
    ...
    # update link node
    link_id2 = create_link(sample_service['url'], TOKEN4,
                           {'id': sample_id1,
                            'version': 1,
                            'node': 'subsample',
                            'upa': '3/1/1',
    ...
    # get expired link
    result = rpc_call_result(sample_service['url'], TOKEN3, 'get_data_links_from_sample', [{
        'id': sample_id1,
        'version': 1,
    ...
    check_kafka_messages(
        kafka_host, [{'event_type': 'NEW_SAMPLE', 'sample_id': sample_id1, 'sample_ver': 1},
...
def test_create_data_link_as_admin(sample_service):
    # create samples
    sample_id1 = create_sample(sample_service['url'], TOKEN3,
                               {'name': 'mysample',
    ...
    # create links
    link_id1 = create_link(sample_service['url'], TOKEN2,
                           {'id': sample_id1,
                            'version': 1,
                            'node': 'root',
                            'upa': '3/1/1',
    ...
    link_id2 = create_link(sample_service['url'], TOKEN2,
                           {'id': sample_id1,
                            'version': 1,
                            'node': 'root',
                            'upa': '3/1/1',
...
def test_get_links_from_sample_exclude_workspaces(sample_service):
    ...
    # create links
    link_id1 = create_link(sample_service['url'], TOKEN3, {'id': sample_id1,
                                                           'version': 1,
                                                           'node': 'root',
                                                           'upa': '4/1/1'}, USER3)
    link_id2 = create_link(sample_service['url'], TOKEN4, {'id': sample_id1,
                                                           'version': 1,
                                                           'node': 'root',
                                                           'upa': '5/1/1'}, USER4)
    link_id3 = create_link(sample_service['url'], TOKEN4, {'id': sample_id1,
                                                           'version': 1,
                                                           'node': 'root',
                                                           'upa': '6/1/1',
                                                           'dataid': 'whee'}, USER4)

    create_link(sample_service['url'], TOKEN4, {'id': sample_id1,
                                                'version': 1,
                                                'node': 'root',
                                                'upa': '7/1/1'}, USER4)

    # check correct links are returned
    result = get_links_from_sample(sample_service['url'], TOKEN3, {'id': sample_id1, 'version': 1})
    ...
    result = get_links_from_sample(sample_service['url'], None, {'id': sample_id1, 'version': 1})
...
def test_get_links_from_sample_as_admin(sample_service):
    ...
    link_id = create_link(sample_service['url'], TOKEN4,
...
def test_get_links_from_sample_public_read(sample_service):
    ...
    link_id = create_link(sample_service['url'], TOKEN1,
                          {'id': sample_id,
                           'version': 1,
                           'node': 'root',
                           'upa': '8/1/1'}, USER1)
    ...
    for token in [None, TOKEN4]:  # anon user & user without explicit permission
        # check correct links are returned
        result = rpc_call_result(sample_service['url'], token,
                                 'get_data_links_from_sample',
...
def test_create_link_param_fail(sample_service):
    ...
    assert_error_create_link(
        sample_service['url'], TOKEN3,
        {'id': sample_id, 'version': 1, 'node': 'root', 'upa': 'upalupa', 'dataid': 'yay'},
...
def test_create_link_fail(sample_service):
    ...
    assert_error_create_link(sample_service['url'], TOKEN3,
...
def test_create_link_fail_link_exists(sample_service):
    ...
    create_link(sample_service['url'], TOKEN3,
                {'id': sample_id,
                 'version': 1,
                 'node': 'root',
                 'upa': '12/1/1',
...
def test_get_links_from_sample_fail(sample_service):
    ...
    def get_link_from_sample_fail(token, params, expected):
        assert_error_rpc_call(sample_service['url'], token,
                              'get_data_links_from_sample',
                              [params], expected)
    ...
    get_link_from_sample_fail(
        ...
        'Sample service error code 30000 Missing input parameter: version')
    get_link_from_sample_fail(
        TOKEN3, {'id': sample_id, 'version': 1, 'effective_time': 'foo'},
    ...
    get_link_from_sample_fail(
        TOKEN4, {'id': sample_id, 'version': 1},
        f'Sample service error code 20000 Unauthorized: User user4 cannot read sample {sample_id}')
    get_link_from_sample_fail(
        None, {'id': sample_id, 'version': 1},
        f'Sample service error code 20000 Unauthorized: Anonymous users cannot read sample {sample_id}')
    badid = uuid.uuid4()
    get_link_from_sample_fail(
        TOKEN3, {'id': str(badid), 'version': 1},
        f'Sample service error code 50010 No such sample: {badid}')

    # admin tests
    get_link_from_sample_fail(
        TOKEN4, {'id': sample_id, 'version': 1, 'as_admin': 1},
    ...
    get_link_from_sample_fail(
        None, {'id': sample_id, 'version': 1, 'as_admin': 1},
...
def test_expire_data_link_fail(sample_service):
    ...
    assert_result_create_link(sample_service['url'], TOKEN3,
                              {'id': sample_id,
                               'version': 1,
                               'node': 'root',
                               'upa': upa,
    ...
    def _expire_data_link_fail(token, params, expected_message, debug=False):
        assert_error_rpc_call(sample_service['url'], token, 'expire_data_link', [params], expected_message, debug=debug)

    # Failure expiration cases
    _expire_data_link_fail(TOKEN3, {},
                           'Sample service error code 30000 Missing input parameter: upa')

    _expire_data_link_fail(TOKEN3, {'upa': '1/0/1'},
                           'Sample service error code 30001 Illegal input parameter: 1/0/1 is not a valid UPA')

    _expire_data_link_fail(TOKEN3, {'upa': '1/1/1', 'dataid': 'foo\nbar'},
                           'Sample service error code 30001 Illegal input parameter: ' +
                           'dataid contains control characters')

    _expire_data_link_fail(TOKEN4, {'upa': '1/1/1', 'dataid': 'yay'},
    ...
    _expire_data_link_fail(TOKEN3, {'upa': bad_upa, 'dataid': data_id},
    ...
    bad_data_id = 'boo'
    _expire_data_link_fail(TOKEN3, {'upa': upa, 'dataid': bad_data_id},
    ...
    _expire_data_link_fail(TOKEN4, {'upa': upa_writable, 'dataid': data_id},
                           (
                               'Sample service error code 20000 Unauthorized: User user4 cannot '
                               f'administrate sample {sample_id}'
                           ))
    ...
    _expire_data_link_fail(TOKEN2, {'upa': upa,
                                    'dataid': data_id,
                                    'as_admin': 1,
                                    'as_user': 'foo\tbar'},
                           'Sample service error code 30001 Illegal input parameter: ' +
...
def test_get_links_from_data(sample_service):
    ...
    link_id1 = assert_result_create_link(sample_service['url'], TOKEN3,
                                         {'id': sample_id1,
                                          'version': 1,
                                          'node': 'root',
                                          'upa': upa2v2}, USER3)

    link_id2 = assert_result_create_link(sample_service['url'], TOKEN4,
                                         {'id': sample_id2,
                                          'version': 1,
                                          'node': 'root',
                                          'upa': upa1,
                                          'dataid': 'column1'}, USER4)

    link_id3 = assert_result_create_link(sample_service['url'], TOKEN4,
                                         {'id': sample_id2,
                                          'version': 2,
                                          'node': 'root',
                                          'upa': upa2v2,
                                          'dataid': 'column2'}, USER4)
...
def test_get_links_from_data_public_read(sample_service):
    ...
    # create links
    link_id = assert_result_create_link(sample_service['url'], TOKEN1,
                                        {'id': sample_id,
                                         'version': 1,
                                         'node': node_id,
                                         'upa': upa}, USER1)
...
def test_get_links_from_data_as_admin(sample_service):
    ...
    username_readadmin = USER3

    # create sample
    sample_id = assert_create_sample(sample_service['url'], token_owner, sample_params, 1)

    # create link
    link_id = assert_result_create_link(sample_service['url'], token_owner,
                                        {'id': sample_id,
                                         'version': 1,
                                         'node': node_id,
                                         'upa': upa},
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token_readadmin)
    result = sample_service_client.call_assert_result('get_data_links_from_data', {'upa': upa, 'as_admin': 1})
...
def test_get_links_from_data_fail(sample_service):
    ...
    def assert_error_get_link_from_data(token, params, expected, debug=False):
        assert_error_rpc_call(sample_service['url'], token, 'get_data_links_from_data', [params], expected, debug)

    # Parameter validation errors
    assert_error_get_link_from_data(token_owner, {},
                                    'Sample service error code 30000 Missing input parameter: upa')

    assert_error_get_link_from_data(token_owner, {'upa': upa_with_object, 'effective_time': 'foo'},
                                    (
                                        "Sample service error code 30001 Illegal input parameter: "
    ...
    # Auth errors
    assert_error_get_link_from_data(token_unauth, {'upa': upa_with_object},
                                    (
                                        'Sample service error code 20000 Unauthorized: '
                                        f'User {username_unauth} cannot read upa {upa_with_object}')
                                    )

    assert_error_get_link_from_data(None, {'upa': upa_with_object},
                                    (
                                        'Sample service error code 20000 Unauthorized: '
                                        f'Anonymous users cannot read upa {upa_with_object}')
                                    )

    assert_error_get_link_from_data(token_owner, {'upa': upa_no_objects},
                                    (
    ...
    assert_error_get_link_from_data(token_unauth, {'upa': upa_with_object, 'as_admin': 1},
                                    (
                                        'Sample service error code 20000 Unauthorized: '
    ...
    assert_error_get_link_from_data(None, {'upa': upa_with_object, 'as_admin': 1},
                                    (
                                        'Sample service error code 20000 Unauthorized: '
                                        'Anonymous users may not act as service administrators.'
                                    ))
    assert_error_get_link_from_data(token_owner, {'upa': upa_no_objects, 'as_admin': 1},
                                    (
                                        'Sample service error code 50040 No such workspace data: '
                                        f'Object {upa_no_objects} does not exist'
                                    ))
    assert_error_get_link_from_data(token_owner, {'upa': upa_non_existent_workspace, 'as_admin': 1},
                                    (
                                        'Sample service error code 50040 No such workspace data: '
...
def test_get_sample_via_data(sample_service):
    ...
    # create samples
    sample_id1 = assert_create_sample(sample_service['url'], token_owner, sample_params_1)
    sample_id2 = assert_create_sample(sample_service['url'], token_owner, sample_params_2)
    case2 = copy.deepcopy(sample_params_2)
    case2['sample']['id'] = sample_id2
    assert_create_sample(sample_service['url'], token_owner, case2, 2)

    # create links
    assert_result_create_link(sample_service['url'], token_owner,
                              {'id': sample_id1,
                               'version': 1,
                               'node': node_id_1,
                               'upa': upa},
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token_user)
    result_01 = sample_service_client.call_assert_result('get_sample_via_data',
                                                         {'upa': upa,
                                                          'id': sample_id1,
                                                          'version': 1})
    ...
    result_02 = sample_service_client.call_assert_result('get_sample_via_data',
                                                         {'upa': upa,
                                                          'id': sample_id2,
                                                          'version': 2})
    ...
    assert result_02 == make_expected_sample(sample_params_2, sample_id2, user_owner, version=2)
...
def test_get_sample_via_data_expired_with_anon_user(sample_service):
    ...
    # update link node
    assert_result_create_link(sample_service['url'], TOKEN1,
                              {
                                  'id': sample_id2,
                                  'version': 1,
                                  'node': node_id_2,
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token_owner)
    result = sample_service_client.call_assert_result('get_sample_via_data',
                                                      {'upa': upa,
                                                       'id': sample_id2,
                                                       'version': 1})
    ...
    assert result == make_expected_sample(sample_params_2, sample_id2, user_owner, version=1)

    # get sample via expired link
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=None)
    result_2 = sample_service_client.call_assert_result('get_sample_via_data',
                                                        {'upa': upa,
                                                         'id': sample_id1,
                                                         'version': 1})
    ...
    assert result_2 == make_expected_sample(sample_params_1, sample_id1, user_owner, version=1)
...
def test_get_sample_via_data_public_read(sample_service):
    ...
    assert result == make_expected_sample(sample_params, sample_id, user_owner, version=1)
...
def test_get_sample_via_data_fail(sample_service):
    ...
    def assert_error_get_sample_via_data(token, params, expected):
        sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token)
    ...
    # Parameter errors
    assert_error_get_sample_via_data(token_owner, {},
                                     'Sample service error code 30000 Missing input parameter: upa')
    assert_error_get_sample_via_data(token_owner, {'upa': upa},
                                     'Sample service error code 30000 Missing input parameter: id')
    assert_error_get_sample_via_data(token_owner, {'upa': upa, 'id': sample_id},
                                     'Sample service error code 30000 Missing input parameter: version')

    # Permissions errors
    assert_error_get_sample_via_data(token_unauth, {'upa': upa, 'id': sample_id, 'version': 1},
                                     (
                                         'Sample service error code 20000 Unauthorized: '
                                         f'User {user_unauth} cannot read upa {upa}')
                                     )
    assert_error_get_sample_via_data(None, {'upa': upa, 'id': sample_id, 'version': 1},
                                     (
                                         'Sample service error code 20000 Unauthorized: '
    ...
    assert_error_get_sample_via_data(token_owner, {'upa': invalid_upa, 'id': sample_id, 'version': 1},
                                     (
                                         'Sample service error code 50040 No such workspace data: '
                                         f'Object {invalid_upa} does not exist')
                                     )

    assert_error_get_sample_via_data(token_owner, {'upa': upa, 'id': invalid_sample_id, 'version': 1},
                                     (
                                         'Sample service error code 50050 No such data link: '
                                         f'There is no link from UPA {upa} to sample {invalid_sample_id}'
                                     ))
    assert_error_get_sample_via_data(token_owner, {'upa': upa, 'id': sample_id, 'version': 2},
                                     (
...
def test_get_data_link(sample_service):
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token_owner)
    result = sample_service_client.call_assert_result('get_data_link', {'linkid': link_id})
    ...
    # expire link
    sample_service_client.call_assert_result('expire_data_link', {'upa': upa, 'dataid': data_id})

    # get link, user 5 has full admin perms
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=token_fulladmin)
    link = sample_service_client.call_assert_result('get_data_link', {'linkid': link_id})
...
def test_get_data_link_fail(sample_service):
    ...
    # Missing parameters
    assert_error_get_data_link(token_owner, {},
                               'Sample service error code 30000 Missing input parameter: linkid')

    # Invalid access
    assert_error_get_data_link(token_unauth, {'linkid': link_id},
                               (
                                   'Sample service error code 20000 Unauthorized: '
    ...
    # Invalid params
    assert_error_get_data_link(token_owner, {'linkid': bad_data_link_id},
                               (
                                   'Sample service error code 50050 No such data link: '
                                   f'{bad_data_link_id}'
                               ))
