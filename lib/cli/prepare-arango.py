...
def create_test_db(arango_host, test_db_name, test_user, test_user_password):
    ...
    systemdb.create_database(test_db_name, [{'username': test_user, 'password': test_user_password}])
...
def try_wait(fun, timeout):
    ...
    while datetime.datetime.now().timestamp() - start < timeout:
        try:
            ...
            print(ex)
            time.sleep(1)
    raise Exception(f'Arango not ready after {datetime.datetime.now().timestamp() - start}s')
