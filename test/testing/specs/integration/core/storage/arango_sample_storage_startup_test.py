...
from common import TEST_COL_SAMPLE, TEST_COL_NODES, TEST_COL_SCHEMA, \
    TEST_COL_DATA_LINK, TEST_COL_WS_OBJ_VER, TEST_COL_NODE_EDGE, TEST_COL_VER_EDGE, TEST_COL_VERSION, nw, dt
...
def assert_init_error(db, colsample, colver, colveredge, colnode, colnodeedge, colws, coldatalink, colschema, now,
                      expected_exception):
    with raises(Exception) as got:
        ArangoSampleStorage(db, colsample, colver, colveredge, colnode, colnodeedge, colws,
                            coldatalink, colschema,
                            now=now)
...
def test_startup_with_extra_config_doc(sample_service):
    ...
    assert_init_error(sample_service['db'], s, v, ve, n, ne, ws, dl, sc, nw, StorageInitError((
...
def test_fail_startup_missing_args(sample_service):
    ...
    def assert_error(colsample, colver, colveredge, colnode, colnodeedge, colws, coldatalink, colschema, now,
                     expected):
        assert_init_error(sample_service['db'], colsample, colver, colveredge, colnode, colnodeedge, colws, coldatalink,
                          colschema, now, MissingParameterError(expected))
    ...
    assert_error(s, v, ve, n, ne, '', dl, sc, nw, 'workspace_object_version_shadow_collection')
...
def test_fail_startup_invalid_args(sample_service):
    ...
    def assert_error(db, colsample, colver, colveredge, colnode, colnodeedge, colws, coldatalink, colschema, now,
                     expected):
        assert_init_error(db, colsample, colver, colveredge, colnode, colnodeedge, colws, coldatalink,
                          colschema, now, ValueError(expected))

    assert_error(None, s, v, ve, n, ne, ws, dl, sc, nw, 'db cannot be a value that evaluates to false')
    assert_error(sample_service['db'], s, v, ve, n, ne, ws, dl, sc, None,
