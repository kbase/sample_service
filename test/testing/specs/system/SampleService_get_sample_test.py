...
from common import \
    replace_acls, get_sample_result, create_sample, assert_create_sample, create_link_assert_result, \
    sample_params_to_sample, create_sample_assert_result, get_sample_assert_error, get_sample_assert_result
from service_client import ServiceClient
from test_cases import CASE_01, CASE_02
from test_constants import TOKEN3, USER6, TOKEN6, USER3, USER1, TOKEN4, TOKEN1, USER4, USER2, TOKEN2
...
def test_get_sample_public_read(sample_service):
    ...
    for token in [TOKEN4, None]:  # unauthed user and anonymous user
        get_sample_assert_result(sample_service['url'], token, {'id': sample['id'], 'version': 1},
...
def test_get_sample_as_admin(sample_service):
    ...
    sample_id = create_sample(sample_service['url'], TOKEN1, {'name': 'mysample',
                                                              'node_tree': [{'id': 'root',
                                                                             'type': 'BioReplicate',
...
def test_get_sample_fail_bad_id(sample_service):
    bad_sample_id = 'foo'
    error = get_sample_assert_error(sample_service['url'], TOKEN2, {'id': bad_sample_id,
                                                                    'version': 1,
                                                                    'as_admin': False})
...
def test_get_sample_fail_permissions(sample_service):
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=TOKEN1)
    sample = sample_service_client.call_assert_result('create_sample', CASE_01)
    sample_id = sample['id']

    error = get_sample_assert_error(sample_service['url'], TOKEN2, {'id': sample_id,
                                                                    'version': 1,
                                                                    'as_admin': False})
    assert error['message'] == (
    ...
    error = get_sample_assert_error(sample_service['url'], None, {'id': sample_id,
                                                                  'version': 1,
                                                                  'as_admin': False})
    assert error['message'] == (
        'Sample service error code 20000 Unauthorized: '
...
def test_get_sample_fail_admin_permissions(sample_service):
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=TOKEN4)
    error = sample_service_client.call_assert_error('get_sample', {'id': sample['id'],
                                                                   'version': 1,
                                                                   'as_admin': True})
    assert error['message'] == (
        'Sample service error code 20000 Unauthorized: '
        f'User {USER4} does not have the necessary administration privileges to run method get_sample'
    )
...
def test_get_sample_via_data(sample_service):
    ...
    create_link_assert_result(sample_service['url'], TOKEN3,
                              {'id': sample1['id'],
                               'version': 1,
                               'node': 'root',
                               'upa': '1/1/1'},
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=TOKEN4)
    sample1_via_data = sample_service_client.call_assert_result('get_sample_via_data', {'upa': '1/1/1',
                                                                                        'id': sample1['id'],
                                                                                        'version': 1})
    ...
    assert sample1_via_data == sample_params_to_sample(CASE_01, {
        'id': sample1['id'],
        'version': 1,
        'user': USER3
    })
    ...
    sample2_via_data = sample_service_client.call_assert_result('get_sample_via_data', {'upa': '1/1/1',
                                                                                        'id': sample2['id'],
                                                                                        'version': 2})
    ...
    assert sample2_via_data == sample_params_to_sample(CASE_02, {
        'id': sample2['id'],
        'user': USER3,
        'version': 2
    })
...
def test_get_sample_via_data_expired_with_anon_user(sample_service):
    ...
    sample2_via_data = sample_service_client.call_assert_result('get_sample_via_data', {'upa': '2/2/2',
                                                                                        'id': sample2['id'],
                                                                                        'version': 1})
    assert_ms_epoch_close_to_now(sample2_via_data['save_date'])
    del sample2_via_data['save_date']
    ...
    sample1_via_data = sample_service_client.call_assert_result('get_sample_via_data', {'upa': '2/2/2',
                                                                                        'id': sample1['id'],
                                                                                        'version': 1})
    assert_ms_epoch_close_to_now(sample1_via_data['save_date'])
    del sample1_via_data['save_date']

    expected_sample = sample_params_to_sample(CASE_01, {
        'id': sample1['id'],
        'user': USER3,
        'version': 1
    })
...
def test_get_sample_via_data_public_read(sample_service, workspace_url):
    ...
    # create links
    create_link_assert_result(sample_service['url'], TOKEN1, {'id': sample1['id'],
                                                              'version': 1,
                                                              'node': 'root',
    ...
    sample_service_client = ServiceClient('SampleService', url=sample_service['url'], token=TOKEN4)
    sample1_via_data = sample_service_client.call_assert_result('get_sample_via_data',
                                                                {'upa': publicly_readable_object_ref,
                                                                 'id': sample1['id'],
                                                                 'version': 1})
