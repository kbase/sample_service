...
from common import rpc_call_result, VER, \
    create_generic_sample, replace_acls, assert_acl_contents, assert_create_sample, assert_error_rpc_call, \
    clear_kafka_messages, update_acls, check_kafka_messages, create_sample_assert_result, \
    sample_params_to_sample, get_sample_assert_result
from service_client import ServiceClient
from test_cases import CASE_01
from test_constants import USER1, TOKEN1, USER2, USER4, USER3, TOKEN4, TOKEN3, TOKEN5, USER5, USER_NO_TOKEN3, \
    USER_NO_TOKEN2, USER_NO_TOKEN1, TOKEN2
...
def test_get_and_replace_acls(sample_service, kafka_host):
    ...
    sample_id = create_sample_assert_result(sample_service['url'], TOKEN1, CASE_01)['id']

    assert_acl_contents(sample_service['url'], sample_id, TOKEN1, {
        'owner': USER1,
        'admin': [],
        'write': [],
        'read': [],
    ...
    replace_acls(sample_service['url'], sample_id, TOKEN1, {
        'admin': [USER2],
    ...
    for token in [TOKEN2, TOKEN3, TOKEN4]:
        assert_acl_contents(sample_service['url'], sample_id, token, {
            'owner': USER1,
            'admin': [USER2],
        ...
        get_sample_assert_result(sample_service['url'], token, {'id': sample_id},
                                 sample_params_to_sample(CASE_01,
                                                         {'user': USER1,
                                                          'id': sample_id,
                                                          'version': 1}))
    ...
    for token, username, version in ((TOKEN2, USER2, 2), (TOKEN3, USER3, 3)):
        ...
        create_sample_assert_result(sample_service['url'], token, params, {'version': version})

        get_sample_assert_result(sample_service['url'], TOKEN1, {'id': sample_id, 'version': version},
                                 sample_params_to_sample(params,
                                                         {'user': username,
                                                          'id': sample_id,
                                                          'version': version}))

    # test that an admin can replace ACLs
    replace_acls(sample_service['url'], sample_id, TOKEN2, {
...
def test_get_acls_as_admin(sample_service):
    ...
    assert_acl_contents(sample_service['url'], sample_id, TOKEN3, {
        'owner': USER1,
        'admin': [],
        'write': [],
        'read': [],
...
def test_get_acls_fail_no_id(sample_service):
    assert_error_rpc_call(sample_service['url'], TOKEN1, 'get_sample_acls', [{}],
...
def test_get_acls_fail_permissions(sample_service):
    ...
    assert_error_rpc_call(sample_service['url'], TOKEN2, 'get_sample_acls', [{'id': sample_id}],
                          f'Sample service error code 20000 Unauthorized: User user2 cannot read sample {sample_id}')

    assert_error_rpc_call(sample_service['url'], None, 'get_sample_acls', [{'id': sample_id}],
                          f'Sample service error code 20000 Unauthorized: Anonymous users cannot read sample {sample_id}')

    assert_error_rpc_call(sample_service['url'], None, 'get_sample_acls', [{'id': sample_id, 'as_admin': 1}],
...
def test_get_acls_fail_admin_permissions(sample_service):
    ...
    assert_error_rpc_call(sample_service['url'], TOKEN4, 'get_sample_acls', [{'id': sample_id, 'as_admin': 1}],
                          (
...
def test_replace_acls_fail_no_id(sample_service):
    assert_error_rpc_call(sample_service['url'], TOKEN1, 'replace_sample_acls', [{}],
                          'Sample service error code 30000 Missing input parameter: id')
...
def test_replace_acls_fail_permissions(sample_service):
    ...
    for user, token in ((USER3, TOKEN3), (USER4, TOKEN4)):
        assert_error_rpc_call(sample_service['url'], token, 'replace_sample_acls',
                              [{'id': sample_id, 'acls': {}}],
                              (
...
def test_replace_acls_fail_bad_user(sample_service):
    ...
    sample_service = ServiceClient('SampleService', url=sample_service['url'], token=TOKEN1)
    invalid_admin_username = 'a'
    invalid_read_username = 'philbin_j_montgomery_iii'

    error = sample_service.call_assert_error('replace_sample_acls',
                                             {'id': sample_id,
                                              'acls': {
...
def test_update_acls(sample_service, kafka_host):
    def assert_update_acls(token, as_admin):
        ...
        update_acls(sample_service['url'], token, {
            'id': sample_id,
            'admin': [USER4],
            'write': [USER2],
            'read': [USER_NO_TOKEN2],
...
def test_update_acls_fail_no_id(sample_service):
    assert_error_rpc_call(sample_service['url'], TOKEN1, 'update_sample_acls', [{}],
                          'Sample service error code 30000 Missing input parameter: id')
...
def test_update_acls_fail_permissions(sample_service):
    ...
    for user, token in ((USER3, TOKEN3), (USER4, TOKEN4)):
        assert_error_rpc_call(sample_service['url'], token, 'update_sample_acls',
                              [{'id': sample_id}],
                              (
...
def test_update_acls_fail_admin_permissions(sample_service):
    sample_id = create_generic_sample(sample_service['url'], TOKEN1)
    for user, token in ((USER3, TOKEN3), (USER4, TOKEN4)):
        assert_error_rpc_call(sample_service['url'], token, 'update_sample_acls', [{'id': sample_id, 'as_admin': 1}],
                              (
                                  f'Sample service error code 20000 Unauthorized: User {user} does not have the '
...
def test_update_acls_fail_bad_user(sample_service):
    ...
    assert_error_rpc_call(sample_service['url'], TOKEN1, 'update_sample_acls',
                          ...
                          (
                              'Sample service error code 50000 No such user: '
                              f'{invalid_admin_user}, {invalid_write_username}, {invalid_read_username}, {invalid_remove_username}'
                          ))
