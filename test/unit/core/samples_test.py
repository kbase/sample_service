...
def test_init_fail_bad_args():
    ...
    mock_sample_storage = create_autospec(ArangoSampleStorage, spec_set=True, instance=True)
    mock_user_lookup = create_autospec(KBaseUserLookup, spec_set=True, instance=True)
    mock_metadata_validator_set = create_autospec(MetadataValidatorSet, spec_set=True, instance=True)
    mock_workspace_client = create_autospec(WS, spec_set=True, instance=True)
    uuid_generator = lambda: UUID('1234567890abcdef1234567890abcdef')  # noqa E731

    _init_fail(None, mock_user_lookup, mock_metadata_validator_set, mock_workspace_client, nw, uuid_generator,
               ValueError(
                   'storage cannot be a value that evaluates to false'))
    _init_fail(mock_sample_storage, None, mock_metadata_validator_set, mock_workspace_client, nw, uuid_generator,
               ValueError(
                   'user_lookup cannot be a value that evaluates to false'))
    _init_fail(mock_sample_storage, mock_user_lookup, None, mock_workspace_client, nw, uuid_generator,
               ValueError(
                   'metadata_validator cannot be a value that evaluates to false'))
    _init_fail(mock_sample_storage, mock_user_lookup, mock_metadata_validator_set, None, nw, uuid_generator,
               ValueError(
                   'workspace cannot be a value that evaluates to false'))
    _init_fail(mock_sample_storage, mock_user_lookup, mock_metadata_validator_set, mock_workspace_client, None,
               uuid_generator,
               ValueError(
                   'now cannot be a value that evaluates to false'))
    _init_fail(mock_sample_storage, mock_user_lookup, mock_metadata_validator_set, mock_workspace_client, nw, None,
               ValueError(
...
def test_replace_sample_acls_with_owner_change():
    ...
    assert storage.replace_sample_acls.call_args_list == [
        ((UUID('1234567890abcdef1234567890abcde0'), SampleACL(u('someuser'), nw(), [u('a')])), {}),
        ((UUID('1234567890abcdef1234567890abcde0'), SampleACL(u('someuser2'), nw(), [u('a')])), {})
    ]
...
def test_replace_sample_acls_with_owner_change_fail_lost_perms():
    ...
    assert storage.replace_sample_acls.call_args_list == [
        ((UUID('1234567890abcdef1234567890abcde0'),
          SampleACL(u('someuser'), nw(), write=[u('b')])), {})
    ]
...
def test_replace_sample_acls_with_owner_change_fail_5_times():
    ...
    assert storage.replace_sample_acls.call_args_list == [
        ((UUID('1234567890abcdef1234567890abcde0'),
          SampleACL(u(f'someuser{x}'), nw(), read=[u('c')]),), {}) for x in range(5)
    ]
...
def _get_links_from_sample(user, public_read=False):
    ...
    ws = create_autospec(WS, spec_set=True, instance=True)
    now = now_fun()
    s = Samples(storage, lu, meta, ws, now=now)
    ...
    assert s.get_links_from_sample(
        user, SampleAddress(UUID('1234567890abcdef1234567890abcdee'), 3)) == ([dl1, dl2], now())
...
def test_get_links_from_data():
    ...
    assert s.get_links_from_data(UserID('u1'), UPA('2/4/6')) == ([dl1, dl2], now())

    ws.has_permission.assert_called_once_with(
        UserID('u1'), WorkspaceAccessType.READ, upa=UPA('2/4/6'))

    storage.get_links_from_data.assert_called_once_with(UPA('2/4/6'), now())
...
def test_get_links_from_data_as_admin():
    ...
    assert s.get_links_from_data(UserID('u1'), UPA('2/4/6'), as_admin=True) == ([dl1, dl2], now())
...
def _expire_data_link(user):
    ...
    kafka = create_autospec(KafkaNotifier, spec_set=True, instance=True)
    now = now_fun()
    s = Samples(storage, lu, meta, ws, kafka, now=now)
    ...
    storage.expire_data_link.assert_called_once_with(now(), user, id_=lid)
...
def test_expire_data_link_as_admin():
    ...
    storage.get_data_link.assert_called_once_with(duid=DataUnitID(UPA('6/1/2'), 'foo'))
    storage.expire_data_link.assert_called_once_with(now(), UserID('userf'), id_=lid)
...
def test_expire_data_link_fail_no_link_at_storage():
    ...
    storage.get_sample_acls.assert_called_once_with(sid)
    storage.expire_data_link.assert_called_once_with(now(), UserID('y'), id_=lid)
